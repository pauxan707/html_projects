...
La API tiene una única entidad `duelos`. Los objetos a guardar dentro de la
misma deben tener la forma:

```javascript
{ id: "some-id", prompt: "Explicame flexbox como si tuviera 5 años", modeloA: "Claude", modeloB: "Gemini", ganador: "A" }
```

Los _endpoints_ a utilizar en el ejercicio son:

+ `GET /duelos` para obtener el historial de duelos guardados.

+ `POST /duelos` para guardar un nuevo duelo con su resultado.
